...
import 'package:flutter_app_comment_system/models/user.dart';

class MockDataProvider with ChangeNotifier {
  List<Comment> getMaximumMockData() {
    List<User> users = [
  ...
  void makeNewComment(String comment) {
    _comments
        .add(Comment(User("New User", true), comment, 233, DateTime.now(), [
      Replay(User("New Reply User", false), "Nice Comment", 1, DateTime.now()),
    ]));
    notifyListeners();
  }
